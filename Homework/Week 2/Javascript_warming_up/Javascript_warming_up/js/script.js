...
// ingredients
shoplist = document.getElementById('ingredient-list').getElementsByTagName('li')
for (i = 0; i < shoplist.length; i++){
	ApplePie.ingredients.push(shoplist[i].textContent)}	
...
document.querySelector(".pie").addEventListener("click", function(){
	console.log("This is an apple pie.");
})
...
function createTransform(domain, range){
	...
	var beta = range[0]; 	//padding
	var alpha = (range[1] - range[0]) / (domain[0] + domain[1]);				//scaling factor
...
console.log(transform(1000)); //should return 15!!
